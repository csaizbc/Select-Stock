...
<main><section class="hero"><b>定位：</b>从全市场寻找趋势、突破与缩量回踩候选。所有指标已在生成时计算，网页只负责筛选排序，因此加载不会重复计算历史行情。<div class="filters"><input id="q" placeholder="代码或名称"><select id="pattern"><option value="">全部形态</option><option>趋势强势</option><option>平台突破</option><option>缩量回踩</option></select><select id="board"><option value="">全部板块</option><option>主板</option><option>创业板</option><option>科创板</option><option>北交所</option></select></div><span id="count"></span></section>
<details class="rules" open><summary>选股逻辑与评分口径（点击展开/收起）</summary><div class="rules-grid">
<div class="rule"><b>基础条件</b><br>非 ST、有最新行情、上市满120日、至少60日历史行情；最终分数不低于55分。</div>
...
<div class="rule"><b>风险扣分</b><br>放量长上影-12；短期涨幅过大-10；量比超过3倍-8。结果仅用于研究，不构成投资建议。</div>
...
const p=JSON.parse(document.getElementById('data').textContent), all=p.rows||[];let key='score',dir=-1;
